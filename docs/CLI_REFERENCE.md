...
| `followup` | `--id`, `--prompt`; optional `--receipt`; `--dry-run`. Live followup consults the same offline doctor receipt as `create-agent` (missing evidence is not a new gate) and then `GET`s agent status. A missing, unknown, or terminal agent blocks the followup POST. `--dry-run` reports `doctor_receipt` / `agent_state=not_checked` / `followup_would_block` and does not GET or POST. |
| `stop-agent` | `--id` |
