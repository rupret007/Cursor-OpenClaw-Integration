...
**One command (recommended, offline):** security sanity + readiness recap + reliability probes, no live OpenClaw model probe:
...
`data/andrea-doctor-receipt.json` that the dashboard, `cursor_handoff`,
`cursor_openclaw.py create-agent`, and `followup` already consume. Pass
`--receipt PATH` only to override that destination.
...
raw probe logs, capability notes, environment values, or the local checkout
path. A failed security/reliability stage forces `overall_status: blocked`,
names `blocked_reason` / `failed_stages`, and overrides leftover Grade A
next-step text to owner-first actions. Consumers must not scrape the file by
hand:
...
The readiness grade leads with a marked operator recap: **Who acts first**,
then **Next for Andrea**, **Next for the coding agent (Bob)**, and **Next for
the owner**, plus fail-closed holds. Details (reasons, summary JSON, routing,
action plan) come after that recap. `andrea_doctor.sh --offline` reprints the
same block at the end of the run. JSON exposes the contract under
`readiness_plan` (`who_acts_first`, `andrea_next_action`,
`coding_agent_next_action`, `owner_next_action`, `holds`, `routing`), so
Codex, Grok, Claude, Bob, dashboards, and shell automation can share blocker
truth without parsing a clipped capability table.
The full doctor receipt adds `stages.security`, `stages.readiness`,
`stages.reliability`, `stages.openclaw_probe`, `blocked_reason`, and
`failed_stages` to that contract. After write, the offline doctor prints a
verify summary. Its offline-only gate prevents a cross-agent handoff command
from accidentally becoming a live model probe.
...
| “What’s the news?” returns an old sprint recap, handoff boilerplate, or tool/config lines | Usually **stale OpenClaw session context** on the host. Defaults: Andrea uses **ephemeral** OpenClaw session ids for structured lookups (live news, recent texts). See **Stale OpenClaw session vs lightweight asks** below. |

### Stale OpenClaw session vs lightweight asks

**Symptom:** A lightweight Telegram ask (news, greetings, capability checks) comes back with unrelated **multi-agent / sprint** narrative or internal strings such as `sessions_spawn` / `attachments.enabled`.

**Cause:** The OpenClaw gateway can keep **long-lived session transcripts** (e.g. under `~/.openclaw/agents/<agent_id>/sessions/`). Those messages are not from Andrea’s SQLite lockstep log; they can still influence model output for hybrid runs.

**Mitigations**

1. **Repo behavior (default):** Structured OpenClaw lookups use a **fresh session id per request** (`andrea-lookup-<route>-<random>`). To debug with stable ids, set `ANDREA_OPENCLAW_LOOKUP_EPHEMERAL_SESSION=0` (restores per-task `andrea-sync-...` session ids).
2. **Host hygiene:** `openclaw gateway restart`. If the problem persists, **back up** then archive or remove the affected session files under `~/.openclaw/agents/<agent_id>/sessions/`. Andrea does not delete these for you.
3. **Config clarity:** Enabling `tools.sessions_spawn.attachments.enabled` is **optional** and only for workflows that spawn sessions **with attachments**—it is **not** required for grounded news or normal Telegram Q&amp;A.
