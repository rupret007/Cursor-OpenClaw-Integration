...
|------|-----------------|--------|
| Binaries | `PATH` | `python3`, `openclaw`, `gh`, `git`, `curl`, optional `gemini`, plus ACP runtime visibility via `acpx` |
...
| ACP router runtime | `openclaw skills list` + `PATH` | If `acp-router` is `✓ ready` but `acpx` is missing, the matrix reports a **blocked** lane with `npm install -g acpx` remediation |
...
- Operator doctor: `bash scripts/andrea_doctor.sh --offline` prints that recap, runs deterministic probes, reprints the recap even on Grade C, and writes `data/andrea-doctor-receipt.json`. Consume with `python3 scripts/andrea_doctor_receipt.py --consume data/andrea-doctor-receipt.json --audience bob`.
