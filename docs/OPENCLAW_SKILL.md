...
4. **Deep operations:** use `cursor_openclaw.py` for `list-agents`, `conversation`, `followup`, `artifacts`, etc. Live `create-agent` and `followup` consult the same offline doctor receipt as `cursor_handoff` before POSTing. Followup also refuses a missing or finished agent. Followup `--dry-run` reports `followup_ready=false` even when the receipt would not block; a successful live followup receipt includes the allowlisted consult.
